...
| 状态 `status` | 前端本地函数 | 按市场交易时段判断，不依赖第三方字段 |
...
| 市场 | 状态逻辑 |
| --- | --- |
| A 股 | 工作日 09:30-11:30、13:00-15:00 为 `trading`，否则 `closed` |
| 港股 | 工作日 09:30-12:00、13:00-16:00 为 `trading`，否则 `closed` |
| 美股 | 按夏令时估算美东工作日 09:30-16:00 为 `trading`，否则 `closed` |

补充：

- A 股和港股另有午休判断函数 `isMarketMiddayBreak`
- 当前没有接入真实节假日历，所以节假日仍可能被误判为普通工作日
...
2. 把节假日、半日市、停牌状态接入真实市场日历，不再只靠本地时间判断。
